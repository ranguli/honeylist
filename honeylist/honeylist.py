...
@click.option(
    "-o", multiple=True, help="Output types. Supports txt, csv, sqlite, and json"
)
def main(i, t, o):
...
def write_wordlist(wordlist, output_file):
    ...
    for output_option in output_file:
        if os.path.exists(output_option):
            sys.exit(
                "{output_option} already exists!".format(output_option=output_option)
            )

    for output_option in output_file:
        file_extension = os.path.splitext(output_option)[-1]
